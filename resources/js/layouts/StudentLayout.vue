...
const fetchUser = async () => {
  try {
    ...
    user.value.role = res.data.role || 'Student'
...
<template>
  <div class="h-screen overflow-hidden">
    ...
    <div class="flex pt-16">
      <StudentSidebar :isOpen="isSidebarOpen" @close="isSidebarOpen = false" />

      <main class="flex-1 p-6 bg-gray-50 overflow-y-auto
         ml-0 xl:ml-64 h-[calc(100vh-4rem)]" :class="isSidebarOpen ? 'pointer-events-none' : ''">
        <slot />
      </main>
    </div>
  </div>
